...
function maior_r(a, nivel) {
    ...
    if (a.length == 1) {
        ...
        var maior_restante = maior_r(a.slice(1), nivel + 1);
        if (a[0] >= maior_restante) {
            return a[0];
        }
        else {
...
var array = [1, 4, 10, 20, -1];
console.log("Array original:\n", array);
var resultado = maior_r(array);
console.log("O maior número do array é:", resultado);

//Fibonacci
function fibonacci(n, nivel) {
    if (nivel === void 0) { nivel = 1; }
    console.log("*".repeat(nivel) + " Chamando fibonacci(".concat(n, ")"));
    if (n <= 1)
        return n;
    return fibonacci(n - 1, nivel + 1) + fibonacci(n - 2, nivel + 1);
}
var n = 5;
console.log("\nFibonacci de ".concat(n, ":\n"));
